...
class FrequencyGame(Tk):
    ...
    def call_back(self, evt):
        ...
        if self.first_click:
            ...
            self.write_text(
                "Now, select another hexagon such that it should be a co-cell of the original hexagon."
            )
            ...
            for _ in range(6):
                ...
                if not self.is_within_bound((reuse_x, reuse_y)):
                    self.write_text(
                        "co-cells are exceeding canvas boundary. Select cell in the center"
                    )
                    self.reset_grid()
                    break
...
if __name__ == '__main__':
    print(
        "Enter i & j values. common (i,j) values are: (1,0), (1,1), (2,0), (2,1), (3,0), (2,2)"
    )
    i = int(input("Enter i: "))
    j = int(input("Enter j: "))
    if i == 0 and j == 0:
        ...
        N = (i**2 + i * j + j**2)
        print("N is {}".format(N))
    game = FrequencyGame(cluster_size=N)
    game.mainloop()